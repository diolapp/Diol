...
import app.diol.voicemail.impl.mail.AuthenticationFailedException;
...
import app.diol.voicemail.impl.mail.Part;
import app.diol.voicemail.impl.mail.internet.BinaryTempFileBody;
import app.diol.voicemail.impl.mail.internet.MimeBodyPart;
import app.diol.voicemail.impl.mail.internet.MimeHeader;
import app.diol.voicemail.impl.mail.internet.MimeMultipart;
import app.diol.voicemail.impl.mail.internet.MimeUtility;
import app.diol.voicemail.impl.mail.store.ImapStore.ImapException;
import app.diol.voicemail.impl.mail.store.ImapStore.ImapMessage;
import app.diol.voicemail.impl.mail.store.imap.ImapConstants;
import app.diol.voicemail.impl.mail.store.imap.ImapElement;
import app.diol.voicemail.impl.mail.store.imap.ImapList;
import app.diol.voicemail.impl.mail.store.imap.ImapResponse;
import app.diol.voicemail.impl.mail.store.imap.ImapString;
...
public class ImapFolder {
     ...
     */
    private static Body decodeBody(Context context, InputStream in, String contentTransferEncoding, int size,
    ...
    private static void parseBodyStructure(ImapList bs, Part part, String id) throws MessagingException {
        if (bs.getElementOrNone(0).isList()) {
                ...
                if (!bodyDispositionParams.isEmpty()) {
                    ...
                    for (int i = 1, count = bodyDispositionParams.size(); i < count; i += 2) {
                        ...
                        contentDisposition.append(String.format(Locale.US, ";\n %s=\"%s\"",
                                bodyDispositionParams.getStringOrEmpty(i - 1).getString().toLowerCase(Locale.US),
            ...
            if (contentDisposition.length() > 0) {
                /*
                 * Set the content disposition containing at least the size. Attachment handling
                 * code will use this down the road.
                 */
                part.setHeader(MimeHeader.HEADER_CONTENT_DISPOSITION, contentDisposition.toString());
            }

            /*
             * Set the Content-Transfer-Encoding header. Attachment code will use this to
             * parse the body.
             */
            if (!encoding.isEmpty()) {
    ...
    public void fetchInternal(Message[] messages, FetchProfile fp, MessageRetrievalListener listener)
        ...
        /*
         * Figure out what command we are going to run: FLAGS - UID FETCH (FLAGS)
         * ENVELOPE - UID FETCH (INTERNALDATE UID RFC822.SIZE FLAGS BODY.PEEK[
         * HEADER.FIELDS (date subject from content-type to cc)]) STRUCTURE - UID FETCH
         * (BODYSTRUCTURE) BODY_SANE - UID FETCH (BODY.PEEK[]<0.N>) where N = max bytes
         * returned BODY - UID FETCH (BODY.PEEK[]) Part - UID FETCH (BODY.PEEK[ID])
         * where ID = mime part ID
         */
        ...
        if (fetchPart != null) {
            ...
            // TODO Why can a single part have more than one Id? And why should we only
            // fetch
        ...
        try {
            connection.sendCommand(
                    String.format(Locale.US, ImapConstants.UID_FETCH + " %s (%s)", ImapStore.joinMessageUids(messages),
                ...
                try {
                    ...
                    if (fetchPart != null) {
                        ...
                        try {
                            ...
                            message.setBody(
                                    decodeBody(store.getContext(), bodyStream, contentTransferEncoding, fetchPart.getSize(), listener));
                        } catch (Exception e) {
    ...
    public void setFlags(Message[] messages, String[] flags, boolean value) throws MessagingException {
        checkOpen();
        ...
        try {
            connection.executeSimpleCommand(
                    String.format(Locale.US, ImapConstants.UID_STORE + " %s %s" + ImapConstants.FLAGS_SILENT + " (%s)",
                            ImapStore.joinMessageUids(messages), value ? "+" : "-", allFlags));
    ...
    /**
     * Selects the folder for use. Before performing any operations on this folder,
     * it must be selected.
     */
    private void doSelect() throws IOException, MessagingException {
        final List<ImapResponse> responses = connection
                .executeSimpleCommand(String.format(Locale.US, ImapConstants.SELECT + " \"%s\"", name));
    ...
    public Quota getQuota() throws MessagingException {
        try {
            final List<ImapResponse> responses = connection
                    .executeSimpleCommand(String.format(Locale.US, ImapConstants.GETQUOTAROOT + " \"%s\"", name));
            ...
            for (ImapResponse response : responses) {
                ...
                for (int i = 0; i < list.size(); i += 3) {
                    ...
                    return new Quota(list.getStringOrEmpty(i + 1).getNumber(-1), list.getStringOrEmpty(i + 2).getNumber(-1));
                }
            }
        } catch (IOException ioe) {
            store.getImapHelper().handleEvent(OmtpEvents.DATA_GENERIC_IMAP_IOE);
