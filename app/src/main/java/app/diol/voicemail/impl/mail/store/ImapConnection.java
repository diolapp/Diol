...
import app.diol.voicemail.impl.mail.CertificateValidationException;
...
import app.diol.voicemail.impl.mail.store.imap.DigestMd5Utils;
import app.diol.voicemail.impl.mail.store.imap.ImapConstants;
import app.diol.voicemail.impl.mail.store.imap.ImapResponse;
import app.diol.voicemail.impl.mail.store.imap.ImapResponseParser;
import app.diol.voicemail.impl.mail.store.imap.ImapUtility;
...
public class ImapConnection {
    ...
    /**
     * Next tag to use. All connections associated to the same ImapStore instance
     * share the same counter to make tests simpler. (Some of the tests involve
     * multiple connections but only have a single counter to track the tag.)
     */
    ...
    void setStore(ImapStore store) {
        // TODO: maybe we should throw an exception if the connection is not closed
        // here,
        // if it's not currently closed, then we won't reopen it, so if the credentials
        // have
    ...
    /**
     * Generates and returns the phrase to be used for authentication. This will be
    ...
    String getLoginPhrase() {
        if (loginPhrase == null) {
            if (imapStore.getUsername() != null && imapStore.getPassword() != null) {
                ...
                loginPhrase = ImapConstants.LOGIN + " " + imapStore.getUsername() + " "
    ...
    public void open() throws IOException, MessagingException {
        ...
        try {
            ...
            if (!response.isOk()) {
                ...
                throw new MessagingException(MessagingException.AUTHENTICATION_FAILED_OR_SERVER_ERROR,
    ...
    /**
     * Closes the connection and releases all resources. This connection can not be
     * used again until {@link #setStore(ImapStore)} is called.
     */
    void close() {
        if (transport != null) {
            logout();
    ...
    private void maybeDoStartTls() throws IOException, MessagingException {
        // STARTTLS is required in the OMTP standard but not every implementation
        // support it.
    ...
    private void doDigestMd5Auth() throws IOException, MessagingException {
        ...
        // The server will issue us a challenge, asking to run MD5 on the nonce with our
        // password
        ...
        // Respond to the challenge. If the server accepts it, it will reply a
        // response-auth which
        // is the MD5 of our password and the cnonce we've provided, to prove the server
        // does know
        // the password.
        //
        ...
        // If failed verifyResponseAuth() will throw a MessagingException, terminating
        // the
        // connection
    ...
    private void queryCapability() throws IOException, MessagingException {
        ...
        capabilities.clear();
        Set<String> disabledCapabilities = imapStore.getImapHelper().getConfig().getDisabledCapabilities();
    ...
    /**
     * Create an {@link ImapResponseParser} from {@code mTransport.getInputStream()}
     * and set it to {@link #parser}.
     *
     * <p>
    ...
    /**
     * Send a single command to the server. The command will be preceded by an IMAP
     * command tag and followed by \r\n (caller need not supply them). Execute a
     * simple command at the server, a simple command being one that is sent in a
     * single line of text
     *
    ...
    public List<ImapResponse> executeSimpleCommand(String command, boolean sensitive)
        ...
        // Currently, we expect callers to do that, but if they fail to we'll be in a
        // broken state.
        sendCommand(command, sensitive);
        return getCommandResponses();
    }

    public String sendCommand(String command, boolean sensitive) throws IOException, MessagingException {
        open();
    ...
    /**
     * Read and return all of the responses from the most recent command sent to the
     * server
     *
     * @return a list of ImapResponses
     * @throws IOException
