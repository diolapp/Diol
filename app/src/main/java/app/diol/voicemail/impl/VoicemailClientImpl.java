...
import app.diol.voicemail.impl.configui.VoicemailSecretCodeActivity;
import app.diol.voicemail.impl.settings.VisualVoicemailSettingsUtil;
import app.diol.voicemail.impl.sync.VvmAccountManager;
import app.diol.voicemail.impl.transcribe.TranscriptionBackfillService;
import app.diol.voicemail.impl.transcribe.TranscriptionConfigProvider;

/**
 * {@link VoicemailClient} to be used when the voicemail module is activated.
 * May only be used above O.
 */
public class VoicemailClientImpl implements VoicemailClient {

    /**
     * List of legacy OMTP voicemail packages that should be ignored. It could never
     * be the active VVM package anymore. For example, voicemails in OC will no
     * longer be handled by telephony, but legacy voicemails might still exist in
     * the database due to upgrading from NYC. Dialer will fetch these voicemails
     * again so it should be ignored.
     */
    ...
    private static final String[] OMTP_VOICEMAIL_TYPE = {TelephonyManager.VVM_TYPE_OMTP, TelephonyManager.VVM_TYPE_CVVM,
    ...
    @Override
    public void setVoicemailEnabled(Context context, PhoneAccountHandle phoneAccountHandle, boolean enabled) {
    ...
    public boolean isVoicemailArchiveAvailable(Context context) {
        ...
        if (!ConfigProviderComponent.get(context).getConfigProvider().getBoolean(ALLOW_VOICEMAIL_ARCHIVE, false)) {
            LogUtil.i("VoicemailClientImpl.isVoicemailArchiveAllowed", "feature disabled by config: %s",
    ...
    @Override
    public void setVoicemailArchiveEnabled(Context context, PhoneAccountHandle phoneAccountHandle, boolean value) {
    ...
    public boolean isVoicemailTranscriptionAvailable(Context context, PhoneAccountHandle phoneAccountHandle) {
        if (phoneAccountHandle == null) {
            LogUtil.i("VoicemailClientImpl.isVoicemailTranscriptionAvailable", "phone account handle is null");
        }

        if (!BuildCompat.isAtLeastO()) {
            LogUtil.i("VoicemailClientImpl.isVoicemailTranscriptionAvailable", "not running on O or later");
            return false;
        }

        if (!isVoicemailEnabled(context, phoneAccountHandle)) {
            LogUtil.i("VoicemailClientImpl.isVoicemailTranscriptionAvailable", "visual voicemail is not enabled");
            return false;
        }

        if (!isActivated(context, phoneAccountHandle)) {
            LogUtil.i("VoicemailClientImpl.isVoicemailTranscriptionAvailable", "visual voicemail is not activated");
            return false;
        }
        ...
        if (!provider.isVoicemailTranscriptionAvailable()) {
            LogUtil.i("VoicemailClientImpl.isVoicemailTranscriptionAvailable", "feature disabled by config");
            return false;
        }
    ...
    public boolean isVoicemailDonationEnabled(Context context, PhoneAccountHandle account) {
        return isVoicemailTranscriptionEnabled(context, account) && isVoicemailDonationAvailable(context, account)
                && VisualVoicemailSettingsUtil.isVoicemailDonationEnabled(context, account);
    }

    @Override
    public void setVoicemailTranscriptionEnabled(Context context, PhoneAccountHandle phoneAccountHandle,
                                                 boolean enabled) {
        Assert.checkArgument(isVoicemailTranscriptionAvailable(context, phoneAccountHandle),
                "transcription must be available before enabling/disabling it");
        VisualVoicemailSettingsUtil.setVoicemailTranscriptionEnabled(context, phoneAccountHandle, enabled);
        if (enabled) {
    ...
    @Override
    public void setVoicemailDonationEnabled(Context context, PhoneAccountHandle phoneAccountHandle, boolean enabled) {
        if (enabled) {
            Assert.checkArgument(
                    ...
                    isVoicemailTranscriptionAvailable(context, phoneAccountHandle),
                    isVoicemailTranscriptionEnabled(context, phoneAccountHandle), phoneAccountHandle.toString());
        }
    ...
    @Override
    public void appendOmtpVoicemailSelectionClause(Context context, StringBuilder where, List<String> selectionArgs) {
        String omtpSource = context.getSystemService(TelephonyManager.class).getVisualVoicemailPackageName();
        if (where.length() != 0) {
            where.append(" AND ");
        }
        where.append("(");
    ...
    @TargetApi(VERSION_CODES.O)
    @Override
    public void appendOmtpVoicemailStatusSelectionClause(Context context, StringBuilder where,
    ...
    public boolean hasAcceptedTos(Context context, PhoneAccountHandle phoneAccountHandle) {
        ...
        if (isVvm3) {
            return preferences.getInt(VoicemailVersionConstants.PREF_VVM3_TOS_VERSION_ACCEPTED_KEY,
                    0) >= VoicemailVersionConstants.CURRENT_VVM3_TOS_VERSION;
        } else {
            return preferences.getInt(VoicemailVersionConstants.PREF_DIALER_TOS_VERSION_ACCEPTED_KEY,
                    0) >= VoicemailVersionConstants.CURRENT_DIALER_TOS_VERSION;
        }
    }
