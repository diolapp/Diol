...
public class DigestMd5Utils {
    ...
    /*
     * response-value = toHex( getKeyDigest ( toHex(getMd5(a1)), { nonce-value, ":"
     * nc-value, ":", cnonce-value, ":", qop-value, ":", toHex(getMd5(a2)) }))
     *
     * @param isResponseAuth is the response the one the server is returning us.
     * response-auth has different a2 format.
     */
    @VisibleForTesting
    static String getResponse(Data data, boolean isResponseAuth) {
        StringBuilder a1 = new StringBuilder();
        a1.append(new String(getMd5(data.username + ":" + data.realm + ":" + data.password), StandardCharsets.ISO_8859_1));
        ...
        return toHex(getKeyDigest(toHex(getMd5(a1.toString())),
                data.nonce + ":" + data.nc + ":" + data.cnonce + ":" + data.qop + ":" + toHex(getMd5(a2.toString()))));
    }
    ...
    /**
     * Let getKeyDigest(k, s) be getMd5({k, ":", s}), i.e., the 16 octet hash of the
    ...
    /**
     * Let toHex(n) be the representation of the 16 octet MD5 hash n as a string of
     * 32 hex digits (with alphabetic characters always in lower case, since MD5 is
     * case sensitive).
     */
    private static String toHex(byte[] n) {
        ...
        for (byte b : n) {
            ...
            result.append(HEX_CHARS.charAt(unsignedByte / 16)).append(HEX_CHARS.charAt(unsignedByte % 16));
        }
    ...
    public static class Data {
        ...
        public void verifyResponseAuth(String response) throws MessagingException {
            ...
            if (!response.substring(RESPONSE_AUTH_HEADER.length()).equals(DigestMd5Utils.getResponse(this, true))) {
        ...
        public String createResponse() {
            ...
            builder.append(DIGEST_CHARSET, CHARSET).appendQuoted(DIGEST_USERNAME, username).appendQuoted(DIGEST_REALM, realm)
                    .appendQuoted(DIGEST_NONCE, nonce).append(DIGEST_NC, nc).appendQuoted(DIGEST_CNONCE, cnonce)
                    .appendQuoted(DIGEST_URI, digestUri).append(DIGEST_RESPONSE, response).append(DIGEST_QOP, qop);
