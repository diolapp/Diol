 ...
 *
 * <p>
 * In essence this is a programmatic representation of the relevant portions of
 * OMTP spec.
 */
public class OmtpConstants {
     ...
     *
     * <p>
     * Each string constant is the field's key in the SMS body which is used by the
     * parser to identify the field's value, if present, in the SMS body.
     */

    /**
     * The event that triggered this SYNC SMS. See
     * {@link OmtpConstants#SYNC_TRIGGER_EVENT_VALUES}
     */
    ...
    public static final String[] PROVISIONING_STATUS_VALUES = {SUBSCRIBER_NEW, SUBSCRIBER_READY, SUBSCRIBER_PROVISIONED,
            SUBSCRIBER_UNKNOWN, SUBSCRIBER_BLOCKED};
    ...
    public static final String[] RETURN_CODE_VALUES = {SUCCESS, SYSTEM_ERROR, SUBSCRIBER_ERROR, MAILBOX_UNKNOWN,
            VVM_NOT_ACTIVATED, VVM_NOT_PROVISIONED, VVM_CLIENT_UKNOWN, VVM_MAILBOX_NOT_INITIALIZED,};
     ...
     *
     * <p>
     * This changes the PIN to access the Telephone User Interface, the traditional
     * voicemail system.
     */
     ...
     *
     * <p>
     * This disables the new user tutorial, the message played to new users calling
     * in the Telephone User Interface.
     */
    ...
    // Alternative form of sync message:
    // MBOXUPDATE?m=<new_message_count>;<key>=<value>;

    public static String getClientType() {
        String manufacturer = truncate(
                android.os.Build.MANUFACTURER.replace('=', '_').replace(';', '_').replace('.', '_').replace(' ', '_'), 12);

        String version = truncate(
                android.os.Build.VERSION.RELEASE.replace('=', '_').replace(';', '_').replace('.', '_').replace(' ', '_'), 8);

        String model = truncate(
                android.os.Build.MODEL.replace('=', '_').replace(';', '_').replace('.', '_').replace(' ', '_'),
