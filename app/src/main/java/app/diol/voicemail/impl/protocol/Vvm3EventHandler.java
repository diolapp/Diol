...
/**
 * Handles {@link OmtpEvents} when {@link Vvm3Protocol} is being used. This
 * handler writes custom error codes into the voicemail status table so support
 * on the dialer side is required.
 *
 * <p>
 * TODO(a bug) disable VVM3 by default so support on system dialer can be
 * ensured.
 */
public class Vvm3EventHandler {
    ...
    // STATUS SMS returned st=U and rc!=2. The user cannot be provisioned and must
    // contact customer
    // support.
    ...
    private static boolean handleConfigurationEvent(Context context, VoicemailStatus.Editor status, OmtpEvents event) {
        switch (event) {
    ...
    private static boolean handleNotificationChannelEvent(VoicemailStatus.Editor unusedStatus, OmtpEvents unusedEvent) {
        return false;
    }
    ...
    private static boolean isPinRandomized(Context context, PhoneAccountHandle phoneAccountHandle) {
        ...
        return VoicemailComponent.get(context).getVoicemailClient().createPinChanger(context, phoneAccountHandle)
                .getScrambledPin() != null;
    }

    @Retention(RetentionPolicy.SOURCE)
    @IntDef({VMS_DNS_FAILURE, VMG_DNS_FAILURE, SPG_DNS_FAILURE, VMS_NO_CELLULAR, VMG_NO_CELLULAR, SPG_NO_CELLULAR,
            VMS_TIMEOUT, VMG_TIMEOUT, STATUS_SMS_TIMEOUT, SUBSCRIBER_BLOCKED, UNKNOWN_USER, UNKNOWN_DEVICE, INVALID_PASSWORD,
            MAILBOX_NOT_INITIALIZED, SERVICE_NOT_PROVISIONED, SERVICE_NOT_ACTIVATED, USER_BLOCKED, IMAP_GETQUOTA_ERROR,
            IMAP_SELECT_ERROR, IMAP_ERROR, VMG_INTERNAL_ERROR, VMG_DB_ERROR, VMG_COMMUNICATION_ERROR, SPG_URL_NOT_FOUND,
            VMG_UNKNOWN_ERROR, PIN_NOT_SET})
