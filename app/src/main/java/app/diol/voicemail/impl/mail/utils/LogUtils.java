...
public class LogUtils {
     ...
     * releases. This should be set to DEBUG for production releases, and VERBOSE
     * for internal builds.
     */
    ...
    public static String contentUriToString(String tag, Uri uri) {
        if (isDebugLoggingEnabled(tag)) {
            ...
            // Debug logging is not enabled, we want to remove the email address from the
            // uri.
     ...
     * error will always be logged at level ASSERT with the call stack. Depending on
     * system configuration, a report may be added to the
