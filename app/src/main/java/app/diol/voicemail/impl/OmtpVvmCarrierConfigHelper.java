...
import app.diol.voicemail.impl.configui.ConfigOverrideFragment;
import app.diol.voicemail.impl.protocol.VisualVoicemailProtocol;
import app.diol.voicemail.impl.protocol.VisualVoicemailProtocolFactory;
import app.diol.voicemail.impl.sms.StatusMessage;
import app.diol.voicemail.impl.sync.VvmAccountManager;

/**
 * Manages carrier dependent visual voicemail configuration values. The primary
 * source is the value retrieved from CarrierConfigManager. If
 * CarrierConfigManager does not provide the config (KEY_VVM_TYPE_STRING is
 * empty, or "hidden" configs), then the value hardcoded in telephony will be
 * used (in res/xml/vvm_config.xml)
 *
 * <p>
 * Hidden configs are new configs that are planned for future APIs, or
 * miscellaneous settings that may clutter CarrierConfigManager too much.
 *
 * <p>
 * The current hidden configs are: {@link #getSslPort()}
 * {@link #getDisabledCapabilities()}
 *
 * <p>
 * TODO(twyen): refactor this to an interface.
 */
...
public class OmtpVvmCarrierConfigHelper {
    ...
    public static final String KEY_VVM_DESTINATION_NUMBER_STRING = CarrierConfigManager.KEY_VVM_DESTINATION_NUMBER_STRING;
    public static final String KEY_VVM_PORT_NUMBER_INT = CarrierConfigManager.KEY_VVM_PORT_NUMBER_INT;
    public static final String KEY_CARRIER_VVM_PACKAGE_NAME_STRING = CarrierConfigManager.KEY_CARRIER_VVM_PACKAGE_NAME_STRING;
    ...
    public static final String KEY_VVM_CELLULAR_DATA_REQUIRED_BOOL = CarrierConfigManager.KEY_VVM_CELLULAR_DATA_REQUIRED_BOOL;
    /**
     * @see #getSslPort()
     */
    ...
    /**
     * Ban a capability reported by the server from being used. The array of string
     * should be a subset of the capabilities returned IMAP CAPABILITY command.
     *
    ...
    @VisibleForTesting
    OmtpVvmCarrierConfigHelper(Context context, PersistableBundle carrierConfig, PersistableBundle telephonyConfig,
    ...
    private static Set<String> getDisabledCapabilities(@Nullable PersistableBundle bundle) {
        ...
        String[] disabledCapabilities = bundle.getStringArray(KEY_VVM_DISABLED_CAPABILITIES_STRING_ARRAY);
    ...
    /**
     * return whether the carrier's visual voicemail is supported, with
     * KEY_VVM_TYPE_STRING set as a known protocol.
     */
    ...
    /**
     * @returns arbitrary String stored in the config file. Used for protocol
     * specific values.
     */
    @Nullable
    ...
    /**
     * For checking upon sim insertion whether visual voicemail should be enabled.
     * This method does so by checking if the carrier's voicemail app is installed.
     */
     ...
     *
     * <p>
     * Sometimes the server states it supports a certain feature but we found they
     * have bug on the server side. For example, in a bug the server reported
     * AUTH=DIGEST-MD5 capability but using it to login will cause subsequent
     * response to be erroneous.
     *
     * @return A set of capabilities that is reported by the IMAP CAPABILITY
     * command, but determined to have issues and should not be used.
     */
    @Nullable
     ...
     * activated, but on the client side all network operations are disabled. SMSs
     * are still monitored so a new message SYNC SMS will be translated to show a
     * message waiting indicator, like traditional voicemails.
     *
     * <p>
     * This is for carriers that does not support VVM deactivation so voicemail can
     * continue to function without the data cost.
     */
    ...
    public void activateSmsFilter() {
        Assert.checkArgument(isValid());
        context.getSystemService(TelephonyManager.class).createForPhoneAccountHandle(getPhoneAccountHandle())
    ...
    public void startProvisioning(ActivationTask task, PhoneAccountHandle phone, VoicemailStatus.Editor status,
                                  StatusMessage message, Bundle data, boolean isCarrierInitiated) {
        Assert.checkArgument(isValid());
    ...
    public String toString() {
        ...
        builder.append("phoneAccountHandle: ").append(phoneAccountHandle).append(", carrierConfig: ")
                .append(carrierConfig != null).append(", telephonyConfig: ").append(telephonyConfig != null).append(", type: ")
                .append(getVvmType()).append(", destinationNumber: ").append(getDestinationNumber())
                .append(", applicationPort: ").append(getApplicationPort()).append(", sslPort: ").append(getSslPort())
                .append(", isEnabledByDefault: ").append(isEnabledByDefault()).append(", isCellularDataRequired: ")
                .append(isCellularDataRequired()).append(", isPrefetchEnabled: ").append(isPrefetchEnabled())
                .append(", isLegacyModeEnabled: ").append(isLegacyModeEnabled()).append("]");
        return builder.toString();
    }

    @Nullable
    private PersistableBundle getCarrierConfig(@NonNull TelephonyManager telephonyManager) {
        CarrierConfigManager carrierConfigManager = (CarrierConfigManager) context
    ...
    /**
     * Suppress the behavior of treating any text attachment with MIME "text/*" as
     * transcription, default to false.
     */
