...
/**
 * Class to subscribe to basic VVM3 visual voicemail, for example, Verizon.
 * Subscription is required when the user is unprovisioned. This could happen
 * when the user is on a legacy service, or switched over from devices that used
 * other type of visual voicemail.
 *
 * <p>
 * The STATUS SMS will come with a URL to the voicemail management gateway. From
 * it we can find the self provisioning gateway URL that we can modify voicemail
 * services.
 *
 * <p>
 * A request to the self provisioning gateway to activate basic visual voicemail
 * will return us with a web page. If the user hasn't subscribe to it yet it
 * will contain a link to confirm the subscription. This link should be clicked
 * through cellular network, and have cookies enabled.
 *
 * <p>
 * After the process is completed, the carrier should send us another STATUS SMS
 * with a new or ready user.
 */
...
public class Vvm3Subscriber {
    ...
    // language=XML
    private static final String VMG_XML_REQUEST_FORMAT = "" + "<?xml version=\"1.0\" encoding=\"UTF-8\"?>"
            + "<VMGVVMRequest>" + "  <MessageHeader>" + "    <transactionid>%1$s</transactionid>" + "  </MessageHeader>"
            + "  <MessageBody>" + "    <mdn>%2$s</mdn>" + "    <operation>%3$s</operation>" + "    <source>Device</source>"
            + "    <devicemodel>%4$s</devicemodel>" + "  </MessageBody>" + "</VMGVVMRequest>";
    ...
    private static final String VVM3_SUBSCRIBE_LINK_DEFAULT_PATTERNS = "["
            + "\"(?i)Subscribe to Basic Visual Voice Mail\"," + "\"(?i)Subscribe to Basic Visual Voicemail\"" + "]";

    private static final int REQUEST_TIMEOUT_SECONDS = 30;

    static {
        // Set the default cookie handler to retain session data for the self
        // provisioning gateway.
        // Note; this is not ideal as it is application-wide, and can easily get
        // clobbered.
        // But it seems to be the preferred way to manage cookie for HttpURLConnection,
        // and manually
    ...
    public Vvm3Subscriber(ActivationTask task, PhoneAccountHandle handle, OmtpVvmCarrierConfigHelper helper,
                          VoicemailStatus.Editor status, Bundle data) {
        ...
        // Assuming getLine1Number() will work with VVM3. For unprovisioned users the
        // IMAP username
        // is not included in the status SMS, thus no other way to get the current phone
        // number.
        number = stripInternational(this.helper.getContext().getSystemService(TelephonyManager.class)
                .createForPhoneAccountHandle(this.handle).getLine1Number());
    }

    /**
     * Self provisioning gateway expects 10 digit national format, but
     * {@link TelephonyManager#getLine1Number()} might return e164 with "+1" at
     * front.
     */
    ...
    static List<Pattern> getSubscribeLinkPatterns(Context context) {
        String patternsJsonString = ConfigProviderComponent.get(context).getConfigProvider()
                .getString(VVM3_SUBSCRIBE_LINK_PATTERNS_JSON_ARRAY, VVM3_SUBSCRIBE_LINK_DEFAULT_PATTERNS);
    ...
    private void processSubscription() {
        try {
            ...
            String subscribeLink = findSubscribeLink(getSubscribeLinkPatterns(helper.getContext()), selfProvisionResponse);
    ...
    /**
     * Get the URL to perform self-provisioning from the voicemail management
     * gateway.
     */
    ...
    /**
     * Sent a request to the self-provisioning gateway, which will return us with a
     * webpage. The page might contain a "Subscribe to Basic Visual Voice Mail" link
     * to complete the subscription. The cookie from this response and cellular data
     * is required to click the link.
     */
    private String getSelfProvisionResponse(String url) throws ProvisioningException {
        ...
        StringRequest stringRequest = new StringRequest(Request.Method.POST, url, future, future) {
            ...
            protected Map<String, String> getParams() {
                ...
                // Language to display the subscription page. The page is never shown to the
                // user
                // so just use English.
    ...
    private void clickSubscribeLink(String subscribeLink) throws ProvisioningException {
        ...
        StringRequest stringRequest = new StringRequest(Request.Method.POST, subscribeLink, future, future);
        requestQueue.add(stringRequest);
        try {
        ...
        // It could take very long for the STATUS SMS to return. Waiting for it is
        // unreliable.
        // Just leave the CONFIG STATUS as CONFIGURING and end the task. The user can
        // always
    ...
    private String vvm3XmlRequest(String operation) throws ProvisioningException {
        ...
        String body = String.format(Locale.US, VMG_XML_REQUEST_FORMAT, transactionId, number, operation, Build.MODEL);

        RequestFuture<String> future = RequestFuture.newFuture();
        StringRequest stringRequest = new StringRequest(Request.Method.POST, voicemailManagementGateway, future, future) {
            @Override
