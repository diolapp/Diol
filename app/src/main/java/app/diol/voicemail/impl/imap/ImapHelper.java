...
import app.diol.voicemail.PinChanger.ChangePinResult;
...
import app.diol.voicemail.impl.VoicemailStatus.Editor;
import app.diol.voicemail.impl.VvmLog;
import app.diol.voicemail.impl.fetch.VoicemailFetchedCallback;
import app.diol.voicemail.impl.mail.Address;
import app.diol.voicemail.impl.mail.Body;
import app.diol.voicemail.impl.mail.BodyPart;
import app.diol.voicemail.impl.mail.FetchProfile;
import app.diol.voicemail.impl.mail.Flag;
...
import app.diol.voicemail.impl.mail.store.ImapConnection;
import app.diol.voicemail.impl.mail.store.ImapFolder;
import app.diol.voicemail.impl.mail.store.ImapFolder.Quota;
import app.diol.voicemail.impl.mail.store.ImapStore;
import app.diol.voicemail.impl.mail.store.imap.ImapConstants;
import app.diol.voicemail.impl.mail.store.imap.ImapResponse;
import app.diol.voicemail.impl.mail.utils.LogUtils;
import app.diol.voicemail.impl.sync.OmtpVvmSyncService.TranscriptionFetchedCallback;

/**
 * A helper interface to abstract commands sent across IMAP interface for a
 * given account.
 */
public class ImapHelper implements Closeable {
    ...
    public ImapHelper(Context context, PhoneAccountHandle phoneAccount, Network network, Editor status)
            throws InitializingException {
        this(context, new OmtpVvmCarrierConfigHelper(context, phoneAccount), phoneAccount, network, status);
    }

    public ImapHelper(Context context, OmtpVvmCarrierConfigHelper config, PhoneAccountHandle phoneAccount,
                      Network network, Editor status) throws InitializingException {
    ...
    @ChangePinResult
    private static int getChangePinResultFromImapResponse(ImapResponse response) throws MessagingException {
     ...
     * @return {@code true} if the operation completes successfully, {@code false}
     * otherwise.
     */
     ...
     *
     * @return A list of voicemail objects containing data about voicemails stored
     * on the server.
     */
    public List<Voicemail> fetchAllVoicemails() {
        ...
        try {
            ...
            // This method retrieves lightweight messages containing only the uid of the
            // message.
    ...
    /**
     * Extract voicemail details from the message structure. Also fetch
     * transcription if a transcription exists.
     */
    private Voicemail getVoicemailFromMessageStructure(MessageStructureWrapper messageStructureWrapper)
        ...
        Voicemail.Builder builder = Voicemail.createForInsertion(time, number).setPhoneAccount(phoneAccount)
                .setSourcePackage(context.getPackageName()).setSourceData(messageDetails.getUid()).setIsRead(isRead)
    ...
    /**
     * The "from" field of a visual voicemail IMAP message is the number of the
     * caller who left the message. Extract this number from the list of "from"
     * addresses.
     *
    ...
    /**
     * Fetches the structure of the given message and returns a wrapper containing
     * the message structure and the transcription structure (if applicable).
     *
    ...
    private MessageStructureWrapper fetchMessageStructure(Message message) throws MessagingException {
        ...
        FetchProfile fetchProfile = new FetchProfile();
        fetchProfile
                .addAll(Arrays.asList(FetchProfile.Item.FLAGS, FetchProfile.Item.ENVELOPE, FetchProfile.Item.STRUCTURE));

        // The IMAP folder fetch method will call "messageRetrieved" on the listener
        // when the
    ...
    /**
     * Fetches the body of the given message and returns the parsed voicemail
     * payload.
     *
    ...
    private void setQuota(ImapFolder.Quota quota) {
        ...
        LogUtils.i(TAG, "Updating Voicemail status table with" + " quota occupied: " + quota.occupied + " new quota total:"
                + quota.total);
    ...
    /**
     * A wrapper to hold a message with its header details and the structure for
     * transcriptions (so they can be fetched in the future).
     */
    ...
    private final class MessageStructureFetchedListener implements ImapFolder.MessageRetrievalListener {
        ...
        /**
         * Check if this IMAP message is a valid voicemail and whether it contains a
         * transcription.
         *
         * @param message The IMAP message.
         * @return The MessageStructureWrapper object corresponding to an IMAP message
         * and transcription.
         */
