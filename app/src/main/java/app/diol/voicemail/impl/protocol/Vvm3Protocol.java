...
import app.diol.voicemail.impl.sms.Vvm3MessageSender;
...
public class Vvm3Protocol extends VisualVoicemailProtocol {
    ...
    /**
     * For VVM3, if the STATUS SMS returns
     * {@link StatusMessage#getProvisioningStatus()} of
     * {@link OmtpConstants#SUBSCRIBER_UNKNOWN} and
     * {@link StatusMessage#getReturnCode()} of this value, the user can
     * self-provision visual voicemail service. For other response codes, the user
     * must contact customer support to resolve the issue.
     */
    ...
    // Standard prompt when the user call into the voicemail, and no prompts when
    // someone else is
    // leaving a voicemail.
    ...
    private static boolean setPin(Context context, PhoneAccountHandle phoneAccountHandle, ImapHelper helper,
                                  StatusMessage message) throws IOException, MessagingException {
        ...
        PinChanger pinChanger = VoicemailComponent.get(context).getVoicemailClient().createPinChanger(context,
                phoneAccountHandle);
    ...
    @Override
    public void startActivation(OmtpVvmCarrierConfigHelper config, @Nullable PendingIntent sentIntent) {
        // VVM3 does not support activation SMS.
        // Send a status request which will start the provisioning process if the user
        // is not
        // provisioned.
    ...
    public void startProvisioning(ActivationTask task, PhoneAccountHandle phoneAccountHandle,
                                  OmtpVvmCarrierConfigHelper config, VoicemailStatus.Editor status, StatusMessage message, Bundle data,
                                  boolean isCarrierInitiated) {
        VvmLog.i(TAG, "start vvm3 provisioning");

        if (isCarrierInitiated) {
            // Carrier can send the "Status UNKNOWN, Can subscribe" status when upgrading to
            // premium VVM.
        ...
        if (OmtpConstants.SUBSCRIBER_UNKNOWN.equals(message.getProvisioningStatus())) {
            ...
            // Save the IMAP credentials in preferences so they are persistent and can be
            // retrieved.
    ...
    public Bundle translateStatusSmsBundle(OmtpVvmCarrierConfigHelper config, String event, Bundle data) {
        // UNRECOGNIZED?cmd=STATUS is the response of a STATUS request when the user is
        // provisioned
        // with iPhone visual voicemail without VoLTE. Translate it into an
        ...
        bundle.putString(OmtpConstants.RETURN_CODE, VVM3_UNKNOWN_SUBSCRIBER_CAN_SUBSCRIBE_RESPONSE_CODE);
