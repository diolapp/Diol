...
/**
 * Events internal to the OMTP client. These should be translated into
 * {@link android.provider.VoicemailContract.Status} error codes before writing
 * into the voicemail status table.
 */
public enum OmtpEvents {
    ...
    CONFIG_DEFAULT_PIN_REPLACED(Type.CONFIGURATION, true), CONFIG_ACTIVATING(Type.CONFIGURATION, true),
    // There are already activation records, this is only a book-keeping activation.
    CONFIG_ACTIVATING_SUBSEQUENT(Type.CONFIGURATION, true), CONFIG_STATUS_SMS_TIME_OUT(Type.CONFIGURATION),
    ...
    // Successfully downloaded/uploaded data from the server, which means the data
    // channel is clear.
    ...
    // All destination address that resolves to the server hostname are rejected or
    // timed out
    DATA_ALL_SOCKET_CONNECTION_FAILED(Type.DATA_CHANNEL),
    // Failed to establish SSL with the server, either with a direct SSL connection
    // or by
    // STARTTLS command
    ...
    DATA_AUTH_UNKNOWN_USER(Type.DATA_CHANNEL), DATA_AUTH_UNKNOWN_DEVICE(Type.DATA_CHANNEL),
    DATA_AUTH_INVALID_PASSWORD(Type.DATA_CHANNEL), DATA_AUTH_MAILBOX_NOT_INITIALIZED(Type.DATA_CHANNEL),
    DATA_AUTH_SERVICE_NOT_PROVISIONED(Type.DATA_CHANNEL), DATA_AUTH_SERVICE_NOT_ACTIVATED(Type.DATA_CHANNEL),
    ...
    VVM3_VMG_DNS_FAILURE, VVM3_SPG_DNS_FAILURE, VVM3_VMG_CONNECTION_FAILED, VVM3_SPG_CONNECTION_FAILED, VVM3_VMG_TIMEOUT,
    VVM3_STATUS_SMS_TIMEOUT,

    VVM3_SUBSCRIBER_PROVISIONED, VVM3_SUBSCRIBER_BLOCKED, VVM3_SUBSCRIBER_UNKNOWN;
