 ...
 *
 * <p>
 * RFC822 email address may have following format. "name" <address> (comment)
 * "name" <address> name <address> address Name and comment part should be
 * MIME/base64 encoded in header if necessary.
 */
public class Address implements Parcelable {
    ...
    /**
     * Regex for the domain part, which is two or more {@link #DOMAIN_PART_PART}
     * separated by .
     */
     ...
     */
    private static final Pattern EMAIL_ADDRESS = Pattern.compile("\\A" + LOCAL_PART + "@" + DOMAIN_PART + "\\z");
    ...
    /**
     * When personal is set, it will return the first token of the personal string.
     * Otherwise, it will return the e-mail address up to the '@' sign.
     */
    ...
    /**
     * Decodes name from UTF-16 string. Optional surrounding double quote will be
     * removed. It will be also unquoted and MIME/base64 decoded.
     *
    ...
    /**
     * This method is used to check that all the addresses that the user entered in
     * a list (e.g. To:) are valid, so that none is dropped.
     */
    @VisibleForTesting
     ...
     * @return A comma-delimited string listing all of the addresses supplied. Null
     ...
     * Address.toString(Address.fromHeader(addressList)).
     */
    @VisibleForTesting
    ...
    /**
     * Returns exactly the same result as
     * Address.toHeader(Address.parse(addressList)).
     */
    @VisibleForTesting
    ...
    /**
     * Returns null if the addressList has 0 addresses, otherwise returns the first
     * address. The same as Address.fromHeader(addressList)[0] for non-empty list.
     * This is an utility method that offers some performance optimization
     * opportunities.
     */
    @VisibleForTesting
    ...
    /**
     * This method exists to convert an address list formatted in a deprecated
     * legacy format to the standard RFC822 header format.
     * {@link #fromHeader(String)} is capable of reading the legacy format and the
     * RFC822 format. {@link #toHeader()} always produces the RFC822 format.
     *
     * <p>
     * This implementation is brute-force, and could be replaced with a more
     * efficient version if desired.
     */
    ...
    /**
     * @param addressList a CSV of RFC822 addresses or the deprecated legacy string
     *                    format
    ...
    public static Address[] fromHeader(String addressList) {
         ...
         * is used, not for every email address; i.e. not for every iteration of the
         * while(). This reduces the theoretical complexity from quadratic to linear,
         * and provides some speed-up in practice by removing redundant scans of the
         * string.
         */
    ...
    /**
     * Returns a simplified string for this e-mail address. When a name is known, it
     * will return the first token of that name. Otherwise, it will return the
     * e-mail address up to the '@' sign.
     */
    ...
    /**
     * Set personal part from UTF-16 string. Optional surrounding double quote will
     ...
     *
     * @return RFC822/MIME compatible address string. It may be surrounded by double
     * quote or quoted and MIME/base64 encoded if necessary.
     */
