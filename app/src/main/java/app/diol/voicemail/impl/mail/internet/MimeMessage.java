 ...
 *
 * <p>
 * NOTE: Automatic generation of a local message-id is becoming unwieldy and
 * should be removed. It would be better to simply do it explicitly on local
 * creation of new outgoing messages.
 */
public class MimeMessage extends Message {
    // Shared random source for generating local message-id values
    private static final java.util.Random random = new java.util.Random();
    // In MIME, en_US-like date format should be used. In other words "MMM" should
    // be encoded to
    // "Jan", not the other localized format like "Ene" (meaning January in locale
    // es).
    // This conversion is used when generating outgoing MIME messages. Incoming MIME
    // date
    // headers are parsed by org.apache.james.mime4j.field.DateTimeField which does
    // not have any
    // localization code.
    ...
    // NOTE: The fields here are transcribed out of headers, and values stored here
    // will supersede
    // the values found in the headers. Use caution to prevent any out-of-phase
    // errors. In
    // particular, any adds/changes/deletes here must be echoed by changes in the
    // parse() function.
    ...
    /**
     * Generate a local message id. This is only used when none has been assigned,
     * and is installed lazily. Any remote (typically server-assigned) message id
     * takes precedence.
     *
    ...
    /**
     * Return the internal mHeader value, with very lazy initialization. The goal is
     * to save memory by not creating the headers until needed.
     */
    ...
    public Date getSentDate() throws MessagingException {
        ...
        if (sentDate == null) {
            ...
            try {
                DateTimeField field = (DateTimeField) DefaultFieldParser
                        .parse("Date: " + MimeUtility.unfoldAndDecode(getFirstHeader("Delivery-date")));
                sentDate = field.getDate();
                // TODO: We should make it more clear what exceptions can be thrown here,
                // and whether they reflect a normal or error condition.
            } catch (Exception e) {
    ...
    /**
     * Returns a list of the given recipient type from this message. If no addresses
     * are found the method returns an empty array.
     */
    @Override
     ...
     * locally-generated random ID, if the value has not previously been set. Local
     * generation can be inhibited/ overridden by explicitly clearing the headers,
     * removing the message-id header, etc.
     *
     * @return the Message-ID header string, or null if explicitly has been set to
     * null
     */
    @Override
     ...
     * @param value header value - flattened by removing CR-NL if any remove header
     *              if value is null
     * @throws MessagingException
     */
    @Override
     ...
     *
     * @return "CR-NL-separated extended headers - null if extended header does not
     * exist
     */
     ...
     *
     * @param headers Extended header and its value - "CR-NL-separated pairs if null
     *                or empty, remove entire extended headers
     * @throws MessagingException
     */
