...
public class VoicemailsQueryHelper {
    ...
    /**
     * Get all the locally deleted voicemails that have not been synced to the
     * server.
     *
     ...
     *
     * <p>
     * TODO(a bug) add PhoneAccountHandle filtering back
     *
     * @param selection A filter declaring which rows to return. {@code null}
     *                  returns all rows.
    ...
    private List<Voicemail> getLocalVoicemails(@NonNull PhoneAccountHandle unusedPhoneAccountHandle, String selection) {
        ...
        try {
            ...
            while (cursor.moveToNext()) {
                ...
                Voicemail voicemail = Voicemail.createForUpdate(id, sourceData).setIsRead(isRead)
    ...
    public void deleteNonArchivedFromDatabase(Voicemail voicemail) {
        contentResolver.delete(Voicemails.CONTENT_URI, Voicemails._ID + "=? AND " + Voicemails.ARCHIVED + "= 0",
    ...
    /**
     * Sends an update command to the voicemail content provider for a list of
     * voicemails. From the view of the provider, since the updater is the owner of
     * the entry, a blank "update" means that the voicemail source is indicating
     * that the server has up-to-date information on the voicemail. This flips the
     * "dirty" bit to "0".
     *
    ...
    /**
     * Voicemail is unique if the tuple of (phone account component name, phone
     * account id, source data) is unique. If the phone account is missing, we also
     * consider this unique since it's simply an "unknown" account.
     *
    ...
    public boolean isVoicemailUnique(Voicemail voicemail) {
        ...
        if (phoneAccount != null) {
            ...
            try {
                String whereClause = Voicemails.PHONE_ACCOUNT_COMPONENT_NAME + "=? AND " + Voicemails.PHONE_ACCOUNT_ID
                        + "=? AND " + Voicemails.SOURCE_DATA + "=?";
    ...
    /**
     * Marks voicemails in the local database as archived. This indicates that the
     * voicemails from the server were removed automatically to make space for new
     * voicemails, and are stored locally on the users devices, without a
    ...
    public List<Voicemail> oldestVoicemailsOnServer(int numVoicemails) {
        ...
        try (Cursor cursor = contentResolver.query(sourceUri, PROJECTION, ARCHIVED_SELECTION, null, sortAndLimit)) {
            ...
            if (voicemails.size() != numVoicemails) {
                Assert.fail(
                        String.format("voicemail count (%d) doesn't matched expected (%d)", voicemails.size(), numVoicemails));
            }
            return voicemails;
        }
    }
